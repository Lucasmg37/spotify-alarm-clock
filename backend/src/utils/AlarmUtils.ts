class AlarmUtils {
  static getNextAlarm(time, nextDay, weekDays): Date {
  ...
  static converArrayWeekPTtoNumber(weekDays: string[]): number[] {
    return weekDays.map((weekDay) => {
      return this.convertWeekNamePtToNumber(weekDay);
    });
  }

  static convertWeekNamePtToNumber(weekName: string): number {
    switch (weekName) {
      case "DOM":
        return 6;
      case "SEG":
